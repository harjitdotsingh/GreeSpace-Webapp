...
public class EPADataServlet extends HttpServlet {
	...
	private static String userNameForService = null;
		...
		if (longitude != null && lat != null) {
			if (appType.equalsIgnoreCase(appTypeiPhone)) {
				...
				jsonString = searchAndReturnDatainJSONFormatForWebApp(
						longitude, lat, radius, maxresults);
			}
	...
	private String searchAndReturnDatainJSONFormat(String zipCode,
		...
		List<EPAToxicDataBean> xmlDataList = (List<EPAToxicDataBean>) pm
				.newQuery(query).execute(zipCode);
		int beanCounter = 1;
		if (xmlDataList != null) {
	...
	static String getUserNameFromFile() {
	...
	@SuppressWarnings("unchecked")
	private String searchAndReturnDatainJSONFormatForWebApp(String sLong,
		...
		sb.append("&username=");
		sb.append(this.getUserNameFromFile());
		...
		try {
			if (zipCodeArray != null) {
				...
				for (String zipCode : zipCodeArray) {
					...
					xmlDataList = (List<EPAToxicDataBean>) pm.newQuery(query)
							.execute(zipCode);
					...
					if (xmlDataList != null) {
						...
						for (EPAToxicDataBean rec : xmlDataList) {
							if (resultCounter < iResult) {
								if (!containsMap.containsKey(rec
										.getFacilityregistryId())) {
									beanList.add(rec);
									++resultCounter;
									containsMap.put(
											rec.getFacilityregistryId(), 0);
			...
			if (beanList.size() > 0) {
				...
				for (int i = 0; i < beanList.size(); ++i) {
					...
					if ((beanCounter != beanList.size() - 1)
							&& beanList.size() > 1) {
						retValuBuffer.append(",");
					}
					++beanCounter;
	...
	@SuppressWarnings("unchecked")
	private String searchAndReturnDatainJSONFormat(String sLong, String sLat,
